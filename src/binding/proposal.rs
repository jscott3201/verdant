...
mod finding_identity_regressions {
    use super::*;
    use crate::binding::findings::{Finding, FindingId};
    ...
    fn identical_binding_preserves_versioned_finding_input() {
        ...
        // Independent literal freezes the version, complete field order and status tail.
        assert_eq!(
            binding.canonical_bytes(),
            "binding-canonical-v2\x1fbacnet://ahu-1\x1fservice\x1fscope-a\x1fahu-1\x1fsupply-air-temp\x1fscope-a\x1fsensor-sat-1\x1fdegC\x1f-\x1fsense\x1fsense\x1fabsent\x1fimported"
        );
        ...
        // Captured from the e4dc077 baseline, not generated by the v2 encoder.
        // Replay's stored-field constructor must retain the old cited record.
        let legacy = Finding::from_stored(
            FindingId::parse("finding-61531dcd8ae4f36c").expect("legacy id"),
            7,
            revision,
            "61531dcd8ae4f36c".to_string(),
            "binding ahu-1:supply-air-temp imported [degC] rev 7 fp fixture-fingerprint".to_string(),
            "fixture-fingerprint".to_string(),
        );
        assert_eq!(legacy.id().as_str(), "finding-61531dcd8ae4f36c");
        assert_eq!(legacy.digest(), "61531dcd8ae4f36c");
        assert_eq!(
            legacy.summary(),
            "binding ahu-1:supply-air-temp imported [degC] rev 7 fp fixture-fingerprint"
        );
    ...
    fn changed_lifecycle_status_cannot_share_finding_identity() {
        ...
        assert_ne!(first.digest(), second.digest(), "imported={first:?}; valid={second:?}");
        assert_ne!(imported.canonical_bytes(), valid.canonical_bytes());
        assert_ne!(first.id(), second.id());
        assert!(valid.canonical_bytes().ends_with("\x1fvalid"));
        assert_eq!(second, Finding::for_binding(&valid, revision, "fixture-fingerprint"));
    }
}
