...
const NOTE_5: &str = "M02-SliceA 0005_action_identity: lossless wire identity and kind";
const NOTE_6: &str = "M02-SliceB 0006_action_lifecycle: durable lifecycle, rate anchor, custody link";
...
fn reference_objects() -> Result<&'static (Objects, Objects, Objects, Objects, Objects, Objects), StorageError> {
    static OBJECT_CACHE: OnceLock<Result<(Objects, Objects, Objects, Objects, Objects, Objects), StorageError>> = OnceLock::new();
    OBJECT_CACHE.get_or_init(|| {
        let script = format!("{}\n{OBJECTS}\nSELECT 'NEXT';\n{}\n{OBJECTS}\nSELECT 'NEXT';\n{}\n{OBJECTS}\nSELECT 'NEXT';\n{}\n{OBJECTS}\nSELECT 'NEXT';\n{}\n{OBJECTS}\nSELECT 'NEXT';\n{}\n{OBJECTS}", super::super::MIGRATION_0001_SQL, super::super::MIGRATION_0002_SQL, super::super::MIGRATION_0003_SQL, super::super::MIGRATION_0004_SQL, super::super::MIGRATION_0005_SQL, super::super::MIGRATION_0006_SQL);
        ...
        let (new3, rest) = rest.split_once("NEXT\n").ok_or_else(|| failure("reference schema 0004 framing"))?;
        let (new4, rest) = rest.split_once("NEXT\n").ok_or_else(|| failure("reference schema 0005 framing"))?;
        let (new5, new6) = rest.split_once("NEXT\n").ok_or_else(|| failure("reference schema 0006 framing"))?;
        let rows = |text: &str| text.lines().map(|l| l.split('|').map(str::to_owned).collect()).collect();
        Ok((rows(old), rows(middle), rows(new3), rows(new4), rows(new5), rows(new6)))
...
/// Returns None only for exactly supported 0001/0002 predecessors. 0003,
/// writer-gated 0004, Slice-A 0005 and Slice-B 0006 are valid currents (fresh
/// open stays at 0003 until the M02-PR06/Slice-A/Slice-B writer ensures apply
/// 0004 then 0005 then 0006). No DDL here.
pub(super) fn validate(conn: &mut Connection) -> Result<Option<String>, StorageError> {
    ...
    let (old, middle, new3, new4, new5, new6) = reference_objects()?;
    if objects != *old && objects != *middle && objects != *new3 && objects != *new4 && objects != *new5 && objects != *new6 { return Err(failure("schema objects differ; refusing without repair")); }
    ...
    let current5 = objects == *new5 && ledger == vec![note(1, NOTE_1), note(2, NOTE_2), note(3, NOTE_3), note(4, NOTE_4), note(5, NOTE_5)];
    let current6 = objects == *new6 && ledger == vec![note(1, NOTE_1), note(2, NOTE_2), note(3, NOTE_3), note(4, NOTE_4), note(5, NOTE_5), note(6, NOTE_6)];
    if (!predecessor && !current3 && !current4 && !current5 && !current6) || app != "1447383636" {
