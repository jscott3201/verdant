...
impl SqliteStore {
    ...
    /// Access-owned SQL batch, guarded inside the admitted writer transaction.
    /// All SQL is generated by access (never caller SQL). The aggregate payload
    /// uses the insert budget; submit/reconcile retain receipts and UNKNOWN.
    /// A failed guard aborts before any durable row, mark, or receipt is written.
    pub(crate) fn prepare_guarded_batch(
        &self, condition: &str, body: &str, payload_bytes: usize,
    ) -> Result<PreparedMutation, StorageError> {
        let sql = format!(
            "CREATE TEMP TABLE access_guard(ok INTEGER NOT NULL CONSTRAINT access_guard_unchanged CHECK(ok=1)); INSERT INTO access_guard VALUES(CASE WHEN ({condition}) THEN 1 ELSE 0 END); {body} SELECT last_insert_rowid(); SELECT changes();"
        );
        let mut pending = PreparedMutation::new(self, &sql, Shape::Insert)?;
    ...
    ///
    /// Fault-injection seam: `.bail on` aborts on the first error and connection
    /// teardown rolls back. Not a product mutation API; tests inspect durability.
    #[cfg(test)]
    pub(crate) fn run_transaction(&self, statements: &[&str]) -> Result<(), StorageError> {
        if statements.is_empty() {
            return Err(StorageError::InvalidInput { what: "transaction", detail: "transaction requires at least one statement".into() });
        }
        let body = format!("BEGIN; {}; COMMIT;", statements.join("; "));
        run_envelope(self.db_path(), self.settings(), &body, self.operation()?).map(|_| ())
    }
